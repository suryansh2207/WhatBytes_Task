...
class PatientDoctorMappingViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        # Check if patient belongs to the current user
        try:
            patient = Patient.objects.get(id=instance.patient.id, created_by=request.user)
        except Patient.DoesNotExist:
            return Response(
                {"error": "Patient not found or you don't have permission to manage this patient's doctors"}, 
                status=status.HTTP_404_NOT_FOUND
            )
